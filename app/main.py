...
def _run(cmd: str) -> int:
    """Run shell command; print stdout/stderr so logs show everything."""
    print(">>>", cmd)
...
async def mix(
    ...
    # Use plain defaults so internal Python calls (like /upload) behave
    ...
    voice_only: Optional[int] = 0,   # IMPORTANT: not a FastAPI Query(...)
    # form fallbacks
    ...
    # Coerce voice_only robustly to 0/1
    try:
...
async def upload_and_mix(
    ...
    # call the same function as /api/mix with form-provided knobs
    return await mix(
